...
class Morbihan(Spider):
    """
    A spider class for crawling the Morbihan department's website for RAA (Recueil des Actes Administratifs) links.
    Inherits from the Spider class.
    """
    def __init__(self, outputDir, configFile, linkFile, date):
        """
        Initialize the Morbihan spider with specific parameters.
        """
        super().__init__(outputDir, configFile, linkFile, date)
        self.baseUrl = "https://www.morbihan.gouv.fr/RAA"
        self.region = "Bretagne"
        self.department = "Morbihan"
        self.currentMostRecentRAA = self.mostRecentRAA
    ...
    def findPages(self, html):
        ...
        for h2 in h2List:
            a = h2.find('a', href=True)
            if a['href'].startswith('/RAA/Annee'):
                annee = a['href'].split('-')[-1]
        ...
        for link in extractedPages:
            i = 0
            while True:
                url = "https://www.morbihan.gouv.fr" + link + "/(offset)/" + str(i*10) # Pagination URL the value of offset is multiplied by 10 to get the next page
    ...
    def extractLinks(self, html, links):
        ...
        for row in rows:
            try:
                ...
                if date > self.mostRecentRAA:
                    link = row['href']
                    links.append({"link": 'https://www.morbihan.gouv.fr' + link, "datePublication": dateStr, "region": self.region, "department": self.department}) # Add the link to the list for the JSON file
                    if date > self.currentMostRecentRAA:
                        self.currentMostRecentRAA = date
