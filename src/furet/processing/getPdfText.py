...
def extractTextList(inputPath, start_page=None, end_page=None):
    """
    Extracts the text between 2 pages (inclusive) of a PDF file. If no start and end pages are defined, then extracts the entire PDF text.
    Returns a list containing the text corresponding to each extracted page
    ...
    Returns:
        list(str): List containing the text corresponding to each extracted page
    """
    ...
    for pageNumber in range(start_page, end_page + 1):
        page = doc.load_page(pageNumber)
        listeTextePages.append(page.get_text())
    
    return listeTextePages